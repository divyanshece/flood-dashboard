...
export default function DashboardPage() {
  ...
  const [downloadInfo, setDownloadInfo] = useState<{ count: number; max: number } | null>(null);

  // Check if user has approved download access via API
  useEffect(() => {
    async function checkDownloadAccess() {
      ...
      try {
        const response = await fetch(`/api/download?userId=${user.id}`);
        const data = await response.json();

        if (data.hasAccess && data.requestId) {
          setCanDownload(true);
          setApprovedRequestId(data.requestId);
          setDownloadInfo({ count: data.downloadCount, max: data.maxDownloads });
        } else {
          setCanDownload(false);
          setApprovedRequestId(null);
          setDownloadInfo(null);
